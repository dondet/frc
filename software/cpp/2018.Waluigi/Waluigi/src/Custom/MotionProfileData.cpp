...
MotionProfileData::MotionProfileData( std::string SA ) {
	channels = 1;
	sets.resize(channels);
	mpSize = 0;
	suffixA = SA;
}

MotionProfileData::MotionProfileData( std::string SA, std::string SB ) {
	channels = 2;
	sets.resize(channels);
	mpSize = 0;
	suffixA = SA;
...
void MotionProfileData::ReadMotionProfile(std::string MP) {
	//Read in csv file(s)
	if( channels > 0 ) { ReadCSV(0, MP + "/" + MP + "_" + suffixA + ".csv"); }
	if( channels > 1 ) { ReadCSV(1, MP + "/" + MP + "_" + suffixB + ".csv"); }
}
